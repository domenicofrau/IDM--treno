...
public class TrenoCriteriaDAO {
	...
	public List<Treno> findByNameLike(String name){
		...
		Predicate namePredicate = criteriaBuilder.like(root.get(("nome").toLowerCase()), "%" + name.toLowerCase() + "%");
	...
	public List<Treno> findTreno(String nome, String marca) {
		...
		Predicate namePredicate = criteriaBuilder.like(root.get(("nome").toLowerCase()), "%" + nome.toLowerCase() + "%");
